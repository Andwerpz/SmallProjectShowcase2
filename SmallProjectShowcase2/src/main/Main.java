...
public class Main {
	public static void main(String[] args) {
		lwjglengine.main.Main.windowTitle = "Small Project Showcase 2";
		lwjglengine.main.Main.initialStateFactory = new MyStateFactory();

		lwjglengine.main.Main.allowWindowResizing = true;
	...
	private static void windowsMakeWallpaper(long window) {
		...
		User32.INSTANCE.MoveWindow(thisWindow, 0, rect.top, rect.right, rect.bottom + 10, false);
		rect.clear();
	}
	...
	private static void linuxMakeWallpaper(long window) {
		...
		X11.INSTANCE.XChangeProperty(display, thisWindow, xAtom(display, "_WIN_LAYER"), X11.XA_CARDINAL, 32, X11.PropModeAppend, memory, 1);

		memory.setNativeLong(0, xAtom(display, "_NET_WM_STATE_BELOW"));
		X11.INSTANCE.XChangeProperty(display, thisWindow, xAtom(display, "_NET_WM_STATE"), X11.XA_ATOM, 32, X11.PropModeAppend, memory, 1);
